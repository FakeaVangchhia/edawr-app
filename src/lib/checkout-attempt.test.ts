...
import { readStorage, resetStorage, seedStorage } from './test-storage';
import {
  basketSignature,
  checkoutAttemptKey,
...
describe('checkoutAttemptKey', () => {
  ...
  it('survives a reload', () => {
    ...
    expect(readStorage(STORAGE_KEY)).toContain(first);
  ...
  it('ignores a stored value of the wrong shape', () => {
    ...
    // anything at all.
    seedStorage(STORAGE_KEY, '{"key":42}');
  ...
  it('still returns a key when storage cannot be written', () => {
    // A full disk, or a storage backend that is simply unavailable. The key
    // still works for retries within this launch because the caller holds it;
    // only the survive-a-relaunch case is lost, and that must not be paid for
    // with an exception on the checkout path.
    //
    // The adapter reports the failure as `false` rather than throwing, which is
    // why this mocks a return value where the web version mocked a throw.
    const setItem = jest.spyOn(storage, 'setItem').mockReturnValue(false);
