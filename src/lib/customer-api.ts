...
/**
 * Register this handset to receive order notifications.
 *
 * **Not present in the storefront** — a browser has no push token to offer, and
 * these two endpoints exist only because this app does. See `src/push.ts` for
 * why registration repeats on every launch, and `edawr-backend/api/push.py` for
 * the contract the server side holds.
 *
 * The device belongs to the *account*, which is why this is an `authRequest`
 * and why a guest gets no notifications: keying a device on a tracking token
 * instead would let anyone holding one subscribe somebody else's phone to it.
 */
export function registerPushToken(
  expoToken: string,
  platform: 'ios' | 'android',
): Promise<void> {
  return authRequest<void>('/api/customer/push-token', {
    method: 'POST',
    body: { expo_token: expoToken, platform },
  });
}

/** Forget this handset, at sign-out. Called *before* the session is cleared. */
export function forgetPushToken(expoToken: string): Promise<void> {
  return authRequest<void>('/api/customer/push-token', {
    method: 'DELETE',
