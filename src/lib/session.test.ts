...
import { resetStorage, seedSecureStorage } from './test-storage';
...
describe('rejecting anything that is not a usable session', () => {
   ...
   *
   * `createLocalStore` parses storage once and then caches the result — that
   * caching is not incidental, it is what stops `useSyncExternalStore` seeing a
   * new object on every render and looping forever, and the factory says so.
   * Writing behind the store's back therefore does nothing to an
   * already-hydrated module. Resetting the module registry is what puts each
   * case back at the un-hydrated state a real launch starts from, which is the
   * only state in which `parse` runs at all.
   *
   * **The keystore, not MMKV.** The session is the one store backed by
   * `expo-secure-store` (see the note at the top of `lib/session.ts`), so
   * seeding the ordinary adapter here would write a key nothing ever reads and
   * every case below would pass for the wrong reason — the store would find
   * nothing and return null, which is what most of them assert.
   */
  const stored = (raw: string) => {
    seedSecureStorage(KEY, raw);
    jest.resetModules();
    const fresh = require('@/lib/session') as typeof import('@/lib/session');
