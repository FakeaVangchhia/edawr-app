 ...
 * here in the same commit. Nothing keeps them in step for you.
 *
 * Trimmed from `edawr-frontend/src/types.ts`: the admin-only shapes (`Product`
 * with its cost price and supplier, `Order`, `User`, `Category`,
 * `AdminSession`) are gone. This app has no route that could receive one, and
 * a type that cannot occur is a type someone will eventually write a component
 * against. `StoreProduct` — no cost price, no supplier, no shelf location, and
 * stock reduced to two booleans — is the only product shape here, which is the
 * `ProductSerializer` / `StoreProductSerializer` split doing its job.
 */
